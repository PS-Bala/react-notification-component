...
function NotificationItem(props: NotificationItemProps) {
    const { items, menuItems, onSelectNotification, enableFloatMenu } = props;
    let sortedItems = items.sort((item1, item2) => (item2.updatedDate.getTime() - item1.updatedDate.getTime()));
    return (
        <>
            {sortedItems.map(((item, index) => (
                <div key={item.id + item.content} className="notification-item" onClick={() => {
                    onSelectNotification && onSelectNotification(item)
                }}>
                    ...
                    {enableFloatMenu && <FloatMenu notificationId={item.id} items={menuItems} viewed={item.viewed} rootClassName={`item-menu ${(index === sortedItems.length - 1) ? "last" : ""}`}></FloatMenu>}
                </div>
            )))}
        </>
    )
